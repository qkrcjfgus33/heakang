...
${'var_idx40_argument'} = new ConditionArgument('var_idx', $args->var_idx, 'equal');
${'var_idx40_argument'}->checkFilter('number');
${'var_idx40_argument'}->createConditionValue();
if(!${'var_idx40_argument'}->isValid()) return ${'var_idx40_argument'}->getErrorMessage();
} else
${'var_idx40_argument'} = NULL;if(${'var_idx40_argument'} !== null) ${'var_idx40_argument'}->setColumnType('number');

${'sort_index41_argument'} = new Argument('sort_index', $args->{'sort_index'});
${'sort_index41_argument'}->ensureDefaultValue('var_idx');
...
,new ConditionWithArgument('`var_idx`',$var_idx40_argument,"equal", 'and')))
));
